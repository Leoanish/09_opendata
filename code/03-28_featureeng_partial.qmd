...
```{r}
fieldweather <- read.csv("../data/field_weatherdata.csv")
...
fe_month <- fieldweather %>%
  ...
  mutate(date_chr = paste0(year, "/", yday),
         date = as.Date(date_chr, format = "%Y/%j"),
         month = month(date),
...
fe_month_sum <- fe_month %>% 
  group_by(year, month, site, month_abb, strength_gtex) %>%
  summarise(across(c(dayl.s, srad.wm2,
                   tmax.c,
                   tmin.c),
...
fe_month_sum_wide <- fe_month_sum %>% 
  ...
  dplyr::select(-c(month, month_abb, name)) %>% 
...
patchwork::wrap_plots(finalplots$plot)
```
